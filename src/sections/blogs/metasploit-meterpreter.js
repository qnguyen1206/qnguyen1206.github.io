export default {
    ...
    category: 'TryHackMe',
    difficulty: 'Easy',
    tags: ['Penetration Testing', 'Metasploit'],
    date: '2026-01-29T12:00:00',
    excerpt: 'Write up and walkthrough of Metasploit: Meterpreter room on TryHackMe.',
    content: `
This is a write up and walkthrough of the Metasploit: Meterpreter room on TryHackMe.
...
**Answer:** speedster
**Reason:**
1. Background the meterpreter session using \`CTRL+Z\`.
2. Run \`use post/windows/gather/enum_shares\`.
3. Run \`set SESSION <session_number>\` where <session_number> is the number of the meterpreter session.
4. Run \`run\`.

What is the NTLM hash of the jchambers user?
**Answer:** 69596c7aa1e8daee17f8e78870e25a5c
**Reason:**
1. Go back to the meterpreter session using \`sessions -i <session_number>\` where <session_number> is the number of the meterpreter session.
2. Run \`hashdump\`.

What is the cleartext password of the jchambers user?
**Answer:** Trustno1
**Reason:** Use online rainbow table and copy the hash from the previous question to find the password.
...
**Reason:** Use \`search -f secrets.txt\`.

What is the Twitter password revealed in the "secrets.txt" file?
**Answer:** KDSvbsw3849!
**Reason:** For some reasons, I can't use \`cat\` command to read so I used \`shell\` to get into the shell and use \`cd c:\\Program Files (x86)\\Windows Multimedia Platform\`. Then I use \`type secrets.txt\` to read the file.

Where is the "realsecret.txt" file located? (Full path of the file)
**Answer:** c:\\inetpub\\wwwroot\\realsecret.txt
**Reason:** Use \`search -f realsecret.txt\`.

What is the real secret?
**Answer:** The Flash is the fastest man alive
**Reason:** For some reasons, I can't use \`cat\` command to read so I used \`shell\` to get into the shell and use \`cd c:\\inetpub\\wwwroot\\realsecret.txt\`. Then I use \`type realsecret.txt\` to read the file.
