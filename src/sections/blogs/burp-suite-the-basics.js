...
**Challenge**
Take a look around the site on \`http://10.65.175.196/\` — we will be using this a lot throughout the module. Visit every other page that is linked on the homepage, then check your sitemap — one endpoint should stand out as being very unusual!
...
**Reason:**
1. Turn \`Intercept is on\` off.
2. Click around the site.
3. Go back to Burp Suite and go to \`Target\` > \`Site map\`.
4. Find the unusual endpoint.
5. Find the flag in the response section.
...
Add http://10.65.175.196/ to your scope and change the proxy settings to only intercept traffic to in-scope targets.
...
You can watch the following video for a visual demonstration of the full certificate import process: Video Not Available
...
We will start by taking a look at the support form at \`http://10.65.175.196/ticket/\`.
