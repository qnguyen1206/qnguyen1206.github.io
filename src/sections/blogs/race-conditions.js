export default {
    ...
    excerpt: 'Write up and walkthrough of Race Conditions room on TryHackMe.',
    content: `
This is a write up and walkthrough of the Race Conditions room on TryHackMe.
...
We cannot be 100% certain which thread will get to update the remaining balance first; however, let’s assume that it is Thread 1. Thread 1 will set the remaining balance to $65. Afterwards, Thread 2 might set the remaining balance to $65 if not appropriately handled. (Thread 2 calculated that $65 should remain in the account after the withdrawal because the balance was $100 when Thread 2 checked it.) In other words, the user made two withdrawals, but the account balance was deducted only for the second one because Thread 2 said so!
...
<img src="/blogs/tryhackme/race-conditions/4bcf6a7618f9a140f0807ae590106b78.svg">
...
Browse to \`http://10.65.189.12:8080\`.
**Note:** The is from the VM attached to this task. It will not be available in this write up.
...
<img src="/blogs/tryhackme/race-conditions/714563c050f0418dd577a28c99ac00e8.png">
...
<img src="/blogs/tryhackme/race-conditions/5b4c02c24aa5d28de8856a161416cb24.png">
...
<img src="/blogs/tryhackme/race-conditions/c74da214d087b71fa9a7cffe37f0d439.png">
...
<img src="/blogs/tryhackme/race-conditions/0eeea41a616bc42b2eff1dbaa35d9483.png">
...
<img src="/blogs/tryhackme/race-conditions/50de5a50b80906df95054b339eb87826.png">
...
**Reason:** Using the technique described above, send money from 1 account to another account, then log in to the other account to get the flag.
...
Browse to \`http://10.65.175.91:5000\`.
**Note:** The is from the VM attached to this task. It will not be available in this write up.
...
**Reason:** Using the technique described above, send money from 1 account to another account, then log in to the other account to get the flag. One thing to keep in mind that instead of 10 or 20 parrallel requests, you need to send somewhere around 1000 (or more) requests to reach the target balance. If one account is not enough to reach the target, use the second one.
`
}