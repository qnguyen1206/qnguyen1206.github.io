...
<img src="/blogs/tryhackme/ids-fundamentals/6645aa8c024f7893371eb7ac-1725532438800.png">
...
**Answer:** 10.11.90.211
**Reason:**
1. Run \`sudo snort -q -l /var/log/snort -r Intro_to_IDS.pcap -A console -c /etc/snort/snort.conf\`.
2. Find the IP Address that appear before the \`->\` symbol.

What other rule message besides the SSH message is detected in the PCAP file? 
**Answer:** Ping Detected
**Reason:**
1. Run \`sudo snort -q -l /var/log/snort -r Intro_to_IDS.pcap -A console -c /etc/snort/snort.conf\`.
2. Keep scrolling down to find the message that is not related to SSH.

What is the sid of the rule that detects SSH?
**Answer:** 1000002
**Reason:**
1. Run \`sudo snort -q -l /var/log/snort -r Intro_to_IDS.pcap -A console -c /etc/snort/snort.conf\`.
2. Find the sid of the rule that detects SSH that appear inside the brackets in the format \`[1:SID_NUMBER:1]\`.
`
}